...
function MessageModal(props) {
    ...
    /**
     * Handles user clicking on group to select it 
     * @param {*} event 
     * @param {*} group : the group object of the group that was clicked on
     */
    const selectGroup = (event, group) => {
        // Check that user hasn't reselected the same group
    ...
    /**
     * Retrieves messages of group from server and returns them, encrypted for particular user 
     * @param {*} group_id : id of group to retrieve messages for 
    ...
    const retrieveMessages = async (group_id) => {
        ...
        try {
            ...
            // retrieve encrypted private key from cookies 
            let encrypted_private_key = Cookies.get('private_key');

            // decrypt key using user password (symetric)
            let private_key = decrypt_key(encrypted_private_key, password);
            
            // Use private key to decrypt each message 
            ...
            setType(2); // change interface to show messages 
            setErrMsg('');
        } catch (err) {
            console.log(err);
    ...
    /**
     * Sends a new message to server to be stored 
     */
    const sendMessage = async () => {
        try {
            ...
            // update usestate variable to show new message in interface 
    ...
    useEffect(() => {
        // Set message scroll to bottom position
